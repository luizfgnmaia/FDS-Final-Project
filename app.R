...
body <- dashboardBody(
  tabItems(
    ...
    tabItem(tabName = "int",
            ...
            HTML("<p> &#8226; Elaborar uma interface que possibilite ao usuário buscar informações sobre um clube em determinado período de tempo."),
            h2("Abordagem"),
...
Acima do 50% de aproveitamento, além do Brasil, temos a Argentina, Colômbia e Equador. Em contrapartida, as equipes da Venezuela são as que demonstram o pior desempenho nos confrontos internacionais com 30,89% no geral e apenas 10,83% em confrontos contra clubes brasileiros.")),
    ...
    tabItem(tabName = "next",
            ...
            HTML("<p> Este trabalho possibilitou o entendimento e aplicação das ideias e procedimentos centrais em um projeto de data science. As ferramentas e pacotes para raspagem de dados, limpeza e tratamento das informações, investigação, analise exploratória e visualização da informação foram cobertas neste trabalho."),
            HTML("<p> Para chegar em uma compreensão desse universo de estudo, definiram-se dois objetivos específicos. O primeiro, construir um ranking mensal dos clubes de futebol da América do Sul utilizando dados históricos desde 2002. Para isso, aplicou-se a metodologia Elo com algumas alterações, o que demandou estudo e adaptação do modelo. Inspirado na abordagem praticada pela Fifa, conseguimos construir um ranking que quantificasse a força relativa entre as equipes e capaz de mostrar essa evolução através do tempo."),
            HTML("<p> Considerando os resultados, verificamos que, de maneira geral, os clubes têm um comportamento muito volátil com o passar das temporadas, sendo pequena a quantidade de equipes que conseguiram se manter no patamar de elite do ranking, tais como São Paulo e Boca Juniors que figuram no topo das tabelas de permanência por meses consecutivos e mais meses na liderança. Um fato que se pode guardar deste estudo é a hegemonia de tais clubes no cenário sul-americano neste século."),
            HTML("<p> Após estudo e análise das partidas e resultados, focamos em alcançar o segundo objetivo: construir uma interface que possibilite ao usuário buscar informações sobre um clube em determinado período de tempo. A aplicação iterativa utilizada apresentou resultados satisfatórios e permitiu com que este objetivo fosse cumprido."),
            HTML("<p> Como próximos passos, gostaríamos de expandir o período considerado pelo ranking coletando partidas de anos anteriores, avaliar a adequabilidade e a qualidade da medida Elo como modelo preditivo de resultados e experimentar outras adaptações desta metodologia."))
